...
namespace gossip_loccs
{
	...
	bool Filter::file_should_be_unpack ( const Method* const m )
	{
		...
		return unpackFilter_.find( BKDRHash(name) ) != unpackFilter_.end();
	}
	...
	bool Filter::object_should_be_traced	( const Object * const obj )
	{
		...
		//all array should be traced
		/*
		const char * s = obj->clazz->descriptor;
		if ( s != NULL && strlen(s) >= 1 && s[0] == '[')
			return true;
		*/
		//need to config array in object.dlist
