...
namespace gossip_loccs
{
	...
	void FuncTracer::record_func ( const Method * const m, const Method * const c, u4 instUid )
	{
		/*
		if ( !(recordFlag & 0x04) )
			return ;
		*/
		...
		//fprintf( traceFile_, "instUid %u||***Caller***||%s||%s||%s\n", instUid, m->clazz->descriptor, m->name, m->shorty);
		fprintf( traceFile_, "instUid %u||", instUid );
		//fprintf( traceFile_, "instUid %d||", instUid );
		fprintf ( traceFile_, "%s||%s||%s\n", c->clazz->descriptor, c->name, c->shorty ); 
		fflush ( traceFile_);
