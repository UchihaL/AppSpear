...
void dumpFileHeader(const DexFile* pDexFile)
{
    ...
    if (pOptHeader != NULL) {
        fprintf(gFp, "Optimized DEX file header:\n");

        asciify(sanitized, pOptHeader->magic, sizeof(pOptHeader->magic));
        fprintf(gFp, "magic               : '%s'\n", sanitized);
        fprintf(gFp, "dex_offset          : %d (0x%06x)\n",
            pOptHeader->dexOffset, pOptHeader->dexOffset);
        fprintf(gFp, "dex_length          : %d\n", pOptHeader->dexLength);
        fprintf(gFp, "deps_offset         : %d (0x%06x)\n",
            pOptHeader->depsOffset, pOptHeader->depsOffset);
        fprintf(gFp, "deps_length         : %d\n", pOptHeader->depsLength);
        fprintf(gFp, "opt_offset          : %d (0x%06x)\n",
            pOptHeader->optOffset, pOptHeader->optOffset);
        fprintf(gFp, "opt_length          : %d\n", pOptHeader->optLength);
        fprintf(gFp, "flags               : %08x\n", pOptHeader->flags);
        fprintf(gFp, "checksum            : %08x\n", pOptHeader->checksum);
        fprintf(gFp, "\n");
    }

    fprintf(gFp, "DEX file header:\n");
    asciify(sanitized, pHeader->magic, sizeof(pHeader->magic));
    fprintf(gFp, "magic               : '%s'\n", sanitized);
    fprintf(gFp, "checksum            : %08x\n", pHeader->checksum);
    fprintf(gFp, "signature           : %02x%02x...%02x%02x\n",
        pHeader->signature[0], pHeader->signature[1],
        pHeader->signature[kSHA1DigestLen-2],
        pHeader->signature[kSHA1DigestLen-1]);
    fprintf(gFp, "file_size           : %d\n", pHeader->fileSize);
    fprintf(gFp, "header_size         : %d\n", pHeader->headerSize);
    fprintf(gFp, "link_size           : %d\n", pHeader->linkSize);
    fprintf(gFp, "link_off            : %d (0x%06x)\n",
        pHeader->linkOff, pHeader->linkOff);
    fprintf(gFp, "string_ids_size     : %d\n", pHeader->stringIdsSize);
    fprintf(gFp, "string_ids_off      : %d (0x%06x)\n",
        pHeader->stringIdsOff, pHeader->stringIdsOff);
    fprintf(gFp, "type_ids_size       : %d\n", pHeader->typeIdsSize);
    fprintf(gFp, "type_ids_off        : %d (0x%06x)\n",
        pHeader->typeIdsOff, pHeader->typeIdsOff);
    fprintf(gFp, "field_ids_size      : %d\n", pHeader->fieldIdsSize);
    fprintf(gFp, "field_ids_off       : %d (0x%06x)\n",
        pHeader->fieldIdsOff, pHeader->fieldIdsOff);
    fprintf(gFp, "method_ids_size     : %d\n", pHeader->methodIdsSize);
    fprintf(gFp, "method_ids_off      : %d (0x%06x)\n",
        pHeader->methodIdsOff, pHeader->methodIdsOff);
    fprintf(gFp, "class_defs_size     : %d\n", pHeader->classDefsSize);
    fprintf(gFp, "class_defs_off      : %d (0x%06x)\n",
        pHeader->classDefsOff, pHeader->classDefsOff);
    fprintf(gFp, "data_size           : %d\n", pHeader->dataSize);
    fprintf(gFp, "data_off            : %d (0x%06x)\n",
...
void dumpOptDirectory(const DexFile* pDexFile)
{
    ...
    fprintf(gFp, "OPT section contents:\n");

    const u4* pOpt = (const u4*) ((u1*) pOptHeader + pOptHeader->optOffset);

    if (*pOpt == 0) {
        fprintf(gFp, "(1.0 format, only class lookup table is present)\n\n");
        return;
    }
    ...
    while (*pOpt != kDexChunkEnd) {
        ...
        fprintf(gFp, "Chunk %08x (%c%c%c%c) - %s (%d bytes)\n", *pOpt,
...
void dumpClassDef(DexFile* pDexFile, int idx)
{
    ...
    fprintf(gFp, "Class #%d header:\n", idx);
    fprintf(gFp, "class_idx           : %d\n", pClassDef->classIdx);
    fprintf(gFp, "access_flags        : %d (0x%04x)\n",
        pClassDef->accessFlags, pClassDef->accessFlags);
    fprintf(gFp, "superclass_idx      : %d\n", pClassDef->superclassIdx);
    fprintf(gFp, "interfaces_off      : %d (0x%06x)\n",
        pClassDef->interfacesOff, pClassDef->interfacesOff);
    fprintf(gFp, "source_file_idx     : %d\n", pClassDef->sourceFileIdx);
    fprintf(gFp, "annotations_off     : %d (0x%06x)\n",
        pClassDef->annotationsOff, pClassDef->annotationsOff);
    fprintf(gFp, "class_data_off      : %d (0x%06x)\n",
        pClassDef->classDataOff, pClassDef->classDataOff);
    fprintf(gFp, "static_fields_size  : %d\n", pClassData->header.staticFieldsSize);
    fprintf(gFp, "instance_fields_size: %d\n",
            pClassData->header.instanceFieldsSize);
    fprintf(gFp, "direct_methods_size : %d\n", pClassData->header.directMethodsSize);
    fprintf(gFp, "virtual_methods_size: %d\n",
...
void dumpCatches(DexFile* pDexFile, const DexCode* pCode)
{
    ...
    if (triesSize == 0) {
        fprintf(gFp, "      catches       : (none)\n");
        return;
    }

    fprintf(gFp, "      catches       : %d\n", triesSize);
...
{
    fprintf(gFp, "        0x%04x - 0x%04x reg=%d %s %s %s\n",
    ...
    int insnWidth, const DecodedInstruction* pDecInsn)
{
    ...
    fprintf(gFp, "%06x:", ((u1*)insns - pDexFile->baseAddr) + insnIdx*2);
    for (i = 0; i < 8; i++) {
        if (i < insnWidth) {
            if (i == 7) {
                fprintf(gFp, " ... ");
                ...
                fprintf(gFp, " %02x%02x", bytePtr[0], bytePtr[1]);
            }
        } else {
            fprintf(gFp, "     ");
        }
    ...
    if (pDecInsn->opcode == OP_NOP) {
        ...
        if (instr == kPackedSwitchSignature) {
            fprintf(gFp, "|%04x: packed-switch-data (%d units)",
                insnIdx, insnWidth);
        } else if (instr == kSparseSwitchSignature) {
            fprintf(gFp, "|%04x: sparse-switch-data (%d units)",
                insnIdx, insnWidth);
        } else if (instr == kArrayDataSignature) {
            fprintf(gFp, "|%04x: array-data (%d units)",
                insnIdx, insnWidth);
        } else {
            fprintf(gFp, "|%04x: nop // spacer", insnIdx);
        }
    } else {
        fprintf(gFp, "|%04x: %s", insnIdx, dexGetOpcodeName(pDecInsn->opcode));
    }
    ...
    case kFmt12x:        // op vA, vB
        fprintf(gFp, " v%d, v%d", pDecInsn->vA, pDecInsn->vB);
        break;
    case kFmt11n:        // op vA, #+B
        fprintf(gFp, " v%d, #int %d // #%x",
            pDecInsn->vA, (s4)pDecInsn->vB, (u1)pDecInsn->vB);
        break;
    case kFmt11x:        // op vAA
        fprintf(gFp, " v%d", pDecInsn->vA);
        break;
    case kFmt10t:        // op +AA
    case kFmt20t:        // op +AAAA
        {
            s4 targ = (s4) pDecInsn->vA;
            fprintf(gFp, " %04x // %c%04x",
                insnIdx + targ,
                (targ < 0) ? '-' : '+',
                (targ < 0) ? -targ : targ);
        }
    ...
    case kFmt21t:        // op vAA, +BBBB
        {
            ...
            fprintf(gFp, " v%d, %04x // %c%04x", pDecInsn->vA,
                insnIdx + targ,
                (targ < 0) ? '-' : '+',
                (targ < 0) ? -targ : targ);
        }
    ...
    case kFmt21h:        // op vAA, #+BBBB0000[00000000]
        ...
        if (pDecInsn->opcode == OP_CONST_HIGH16) {
            ...
            fprintf(gFp, " v%d, #long %lld // #%x",
                pDecInsn->vA, value, (u2)pDecInsn->vB);
        }
        break;
    case kFmt21c:        // op vAA, thing@BBBB
    case kFmt31c:        // op vAA, thing@BBBBBBBB
        fprintf(gFp, " v%d, %s", pDecInsn->vA, indexBuf);
        break;
    case kFmt23x:        // op vAA, vBB, vCC
        fprintf(gFp, " v%d, v%d, v%d", pDecInsn->vA, pDecInsn->vB, pDecInsn->vC);
        break;
    case kFmt22b:        // op vAA, vBB, #+CC
        fprintf(gFp, " v%d, v%d, #int %d // #%02x",
            pDecInsn->vA, pDecInsn->vB, (s4)pDecInsn->vC, (u1)pDecInsn->vC);
        break;
    case kFmt22t:        // op vA, vB, +CCCC
        {
            s4 targ = (s4) pDecInsn->vC;
            fprintf(gFp, " v%d, v%d, %04x // %c%04x", pDecInsn->vA, pDecInsn->vB,
                insnIdx + targ,
                (targ < 0) ? '-' : '+',
                (targ < 0) ? -targ : targ);
        }
    ...
    case kFmt22s:        // op vA, vB, #+CCCC
        fprintf(gFp, " v%d, v%d, #int %d // #%04x",
            pDecInsn->vA, pDecInsn->vB, (s4)pDecInsn->vC, (u2)pDecInsn->vC);
        break;
    case kFmt22c:        // op vA, vB, thing@CCCC
    case kFmt22cs:       // [opt] op vA, vB, field offset CCCC
        fprintf(gFp, " v%d, v%d, %s", pDecInsn->vA, pDecInsn->vB, indexBuf);
        break;
    case kFmt30t:
        fprintf(gFp, " #%08x", pDecInsn->vA);
        break;
    ...
    case kFmt35mi:       // [opt] inline invoke
        {
            ...
            for (i = 0; i < (int) pDecInsn->vA; i++) {
                if (i == 0)
                    fprintf(gFp, "v%d", pDecInsn->arg[i]);
                else
                    fprintf(gFp, ", v%d", pDecInsn->arg[i]);
            }
            fprintf(gFp, "}, %s", indexBuf);
        }
        break;
    ...
    case kFmt51l:        // op vAA, #+BBBBBBBBBBBBBBBB
        {
            ...
            fprintf(gFp, " v%d, #double %f // #%016llx",
...
void dumpCode(DexFile* pDexFile, const DexMethod* pDexMethod)
{
    ...
    fprintf(gFp, "      registers     : %d\n", pCode->registersSize);
    fprintf(gFp, "      ins           : %d\n", pCode->insSize);
    fprintf(gFp, "      outs          : %d\n", pCode->outsSize);
    fprintf(gFp, "      insns size    : %d 16-bit code units\n", pCode->insnsSize);
...
void dumpMethod(DexFile* pDexFile, const DexMethod* pDexMethod, int i)
{
    ...
    if (gOptions.outputFormat == OUTPUT_PLAIN) {
        ...
        fprintf(gFp, "      name          : '%s'\n", name);
        fprintf(gFp, "      type          : '%s'\n", typeDescriptor);
        fprintf(gFp, "      access        : 0x%04x (%s)\n",
            pDexMethod->accessFlags, accessStr);

        if (pDexMethod->codeOff == 0) {
            fprintf(gFp, "      code          : (none)\n");
        } else {
            fprintf(gFp, "      code          -\n");
        ...
        if (constructor) {
            ...
            fprintf(gFp, "<constructor name=\"%s\"\n", tmp);
            free(tmp);
            ...
            fprintf(gFp, " return=\"%s\"\n", tmp);
            free(tmp);

            fprintf(gFp, " abstract=%s\n",
                quotedBool((pDexMethod->accessFlags & ACC_ABSTRACT) != 0));
            fprintf(gFp, " native=%s\n",
            ...
            fprintf(gFp, " synchronized=%s\n", quotedBool(isSync));
        }

        fprintf(gFp, " static=%s\n",
        ...
        while (*base != ')') {
            ...
            *cp++ = '\0';
            ...
            fprintf(gFp, "<parameter name=\"arg%d\" type=\"%s\">\n</parameter>\n",
                argNum++, tmp);
            free(tmp);
        }

        if (constructor)
            fprintf(gFp, "</constructor>\n");
        else
            fprintf(gFp, "</method>\n");
    }
...
void dumpClass(DexFile* pDexFile, int idx, char** pLastPackage)
{
    ...
    if (pClassData == NULL) {
        fprintf(gFp, "Trouble reading class data (#%d)\n", idx);
        goto bail;
    }
    ...
    if (gOptions.outputFormat == OUTPUT_PLAIN) {
        fprintf(gFp, "Class #%d            -\n", idx);
        fprintf(gFp, "  Class descriptor  : '%s'\n", classDescriptor);
        fprintf(gFp, "  Access flags      : 0x%04x (%s)\n",
            pClassDef->accessFlags, accessStr);

        if (superclassDescriptor != NULL)
            fprintf(gFp, "  Superclass        : '%s'\n", superclassDescriptor);

        fprintf(gFp, "  Interfaces        -\n");
    } else {
        char* tmp;

        tmp = descriptorClassToDot(classDescriptor);
        fprintf(gFp, "<class name=\"%s\"\n", tmp);
        free(tmp);

        if (superclassDescriptor != NULL) {
            tmp = descriptorToDot(superclassDescriptor);
            fprintf(gFp, " extends=\"%s\"\n", tmp);
            free(tmp);
        }
        fprintf(gFp, " abstract=%s\n",
    ...
    if (gOptions.outputFormat == OUTPUT_PLAIN) {
        fprintf(gFp, "  source_file_idx   : %d (%s)\n",
...
void dumpDifferentialCompressedMap(const u1** pData)
{
    ...
    fprintf(gFp, "        (differential compression %d -> %d [%d -> %d])\n",
        origLen, compLen,
...
void dumpRegisterMaps(DexFile* pDexFile)
{
    ...
    if (pClassPool == NULL) {
        fprintf(gFp, "No register maps found\n");
        return;
    }
    ...
    fprintf(gFp, "RMAP begins at offset 0x%07x\n", baseFileOffset);
    fprintf(gFp, "Maps for %d classes\n", numClasses);
...
void processDexFile( DexFile* pDexFile)
{
    ...
    if (gOptions.verbose) {
        fprintf(gFp, "Opened DEXFILE, DEX version '%.3s'\n",
...
void processDexdump(FILE* fp, DexFile* pDexFile)
{
	...
	//ALOG(LOG_VERBOSE,"LBD","AAAAAAAAAAAAAAA");
