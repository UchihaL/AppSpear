...
//LBD
void monitor_func_call( art::mirror::ArtMethod* method, const char * currentclass) SHARED_LOCKS_REQUIRED(art::Locks::mutator_lock_);

void monitor_parameter( art::mirror::ArtMethod* method, art::ShadowFrame& shadow_frame, const Instruction* inst, uint16_t inst_data, bool is_range, const char * currentclass) SHARED_LOCKS_REQUIRED(art::Locks::mutator_lock_);

void unpack_dex(art::mirror::ArtMethod* method, art::mirror::ArtMethod* sf_method); //SHARED_LOCKS_REQUIRED(art::Locks::mutator_lock_);
...
static inline bool DoInvoke(Thread* self, ShadowFrame& shadow_frame, const Instruction* inst,
  ...
  if (UNLIKELY(method == nullptr)) {
    ...
    //LBD
    monitor_func_call (method, sf_method->GetDeclaringClassDescriptor());
    monitor_parameter(method,shadow_frame,inst,inst_data,is_range, sf_method->GetDeclaringClassDescriptor());
    unpack_dex(method, sf_method);
...
static inline bool DoInvokeVirtualQuick(Thread* self, ShadowFrame& shadow_frame,
  ...
  //LOG(WARNING) << "UCHIHALART  virtualmethod: "<<method->GetDeclaringClassDescriptor()<<method->GetName();
