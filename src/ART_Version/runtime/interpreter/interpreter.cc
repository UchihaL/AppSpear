...
static void InterpreterJni(Thread* self, ArtMethod* method, const StringPiece& shorty,
  ...
  LOG(WARNING) << "UCHIHALART InterpreterJni";
...
//LBD
  //LOG(WARNING) << "UCHIHALART  !method trace: "<<method->GetName();
  //gossip::diaos_monitor_parameter(method->GetDeclaringClassDescriptor(),method->GetName());
...
/*
  if(gossip::diaos_check()){
      ...
      ArtMethod* method1 = shadow_frame->GetMethod();
    
      char flag[256] = {0};
      uint32_t shorty_len = 0;
      snprintf ( flag, 256, "%s%s%s", method1->GetDeclaringClassDescriptor(), method1->GetName(), method1->GetShorty(&shorty_len) );


      char address[256] = {0};
      snprintf ( address, 256, "ep: %ld  code: %ld  %s", (long)gossip::getEP(flag), (long)method1->GetQuickOatCodeOffset(), method1->GetName());
      LOG(WARNING) << "UCHIHALART ep  "<< address;


      method1->SetEntryPointFromInterpreter(artInterpreterToCompiledCodeBridge);
      //method1->GetQuickOatCodeOffset()
      //method1->SetEntryPointFromQuickCompiledCode((void*)(long)method1->GetQuickOatCodeOffset());
      method1->SetEntryPointFromQuickCompiledCode(gossip::getEP(flag));
      //artInterpreterToCompiledCodeBridge(self, mh, code_item, shadow_frame, result);
      //return;
    //}
  }
*/
