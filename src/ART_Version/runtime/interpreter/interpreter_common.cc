...
//LBD
void monitor_func_call( art::mirror::ArtMethod* method, const char * currentclass){
  
  gossip::diaos_monitor_func_call( method, currentclass);
}
void monitor_parameter( art::mirror::ArtMethod* method, art::ShadowFrame& shadow_frame, const Instruction* inst, uint16_t inst_data, bool is_range, const char * currentclass){
  gossip::diaos_monitor_parameter_art(method, shadow_frame, inst, inst_data, is_range, currentclass);
}

void unpack_dex(art::mirror::ArtMethod* method, art::mirror::ArtMethod* sf_method){
  gossip::diaos_unpack(method, sf_method);
}
...
bool DoCall(ArtMethod* method, Thread* self, ShadowFrame& shadow_frame,
  ...
  //if(gossip::diaos_check()){
      ...
      //method->SetEntryPointFromQuickCompiledCode((void*)(long)method->GetQuickOatCodeOffset());
      //method->GetQuickOatCodeOffset()
  //}
