...
void ClassLinker::LinkCode(Handle<mirror::ArtMethod> method, const OatFile::OatClass* oat_class,
  ...
  if(gossip::diaos_check()){
    //LOG(WARNING) << "UCHIHALART  check ok"<< method->GetDeclaringClassDescriptor()<<" "<<method->GetName();
  ...
  //if(gossip::diaos_check()){
      //if(!method->IsNative() && !method->IsAbstract() && !method->IsStatic()) {
          //if(gossip::class_should_be_traced(method->GetDeclaringClassDescriptor())){
            //char flag[256] = {0};
            //uint32_t shorty_len = 0;
            //snprintf ( flag, 256, "%s%s%s", method->GetDeclaringClassDescriptor(), method->GetName(), method->GetShorty(&shorty_len) );
            //gossip::setEP(flag, method->GetEntryPointFromQuickCompiledCode());

            //char address[256] = {0};
            //snprintf ( address, 256, "ep: %ld  code: %d", (long)method->GetEntryPointFromQuickCompiledCode(), method->GetQuickOatCodeOffset());
